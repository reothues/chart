...
class View {
    ...
    scaleX(x) { return (x * this.unitX + this.offsetX); }
    scaleY(y) { return (y * this.unitY + this.offsetY); }
    scaleXY(x, y) { return [this.scaleX(x), this.scaleY(y)]; }
    rscaleX(x) { return Math.round((x - this.offsetX) / this.unitX); }
    rscaleY(y) { return Math.round((y - this.offsetY) / this.unitY); }
    rscaleXY(x, y) { return [this.rscaleX(x), this.rscaleY(y)]; }
    get scale() {
        return this.c.scale;
    }
    get viewbox() {
    ...
    getView(el) {
        let ret = new chart_1.Viewbox(el.bBox);
        ...
        switch (el.type) {
            case "curve":
                ...
                if (el.points.length <= 0) {
                    ...
                    for (let i = 2; i < el.points.length - 2; i += 2) {
                        ...
                        let dx = nextpoint[0] - lastpoint[0], dy = nextpoint[1] - lastpoint[1];
                        let rate = smoothness / Math.hypot(dx, dy);
                        dx *= rate;
                        dy *= rate;
                        points.push((currentpoint[0] - dx).toFixed(2), (currentpoint[1] - dy).toFixed(2), currentpoint[0].toFixed(2), currentpoint[1].toFixed(2), 'C', (currentpoint[0] + dx).toFixed(2), (currentpoint[1] + dy).toFixed(2));
                        lastpoint = currentpoint;
                    }
                    let currentpoint = [this.scaleX(el.points[el.points.length - 2]), this.scaleY(el.points[el.points.length - 1])];
                    let dx = currentpoint[0] - lastpoint[0], dy = currentpoint[1] - lastpoint[1];
                    let rate = smoothness / Math.hypot(dx, dy);
                    dx *= rate;
                    dy *= rate;
                    points.push((currentpoint[0] - dx).toFixed(2), (currentpoint[1] - dy).toFixed(2), (currentpoint[0]).toFixed(2), currentpoint[1].toFixed(2));
            ...
            case 'line': {
                let points = ['M', ...this.scaleXY(el.points[0], el.points[1])];
