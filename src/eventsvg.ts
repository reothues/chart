...
import { NS_SVG, SVGChart as Raw, svgDeafElement } from './drawsvg';

export class SVGChart extends Raw {
  mark: SVGSVGElement;
  label: SVGGElement;
  monkey: SVGSVGElement;
  SPEED = 14085;
  Zdistance = 100;
  XRotation = -25;
  YRotation = 10;
  activeDot: DeafElement;
  get is3DActived () : boolean {
    return (this.monkey.style.display != 'none')
  }
  set is3DActived(is : boolean) {
  ...
  renderMaker(svgNode: SVGSVGElement) : DeafElement {
    let mark = new DeafElement('g');
    mark.attributes.svgNode = svgNode;
    let marker = mark.appendChild(new DeafLine) as DeafLine;
  ...
  renderMonkey(svgNode: SVGSVGElement) : DeafElement {
    let mark = new DeafElement('g');
    ...
    let vpath = mark.appendChild(new DeafLine) as DeafLine;
    vpath.setAttribute('class', 'monkey')
    vpath.lineTo(0, 0);
    this.renderSVG(vpath);

    Object.keys(this.watch.members).forEach(v => {
      vpath.attributes.svgNode.appendChild(document.createElementNS(NS_SVG, 'circle'));
    })

    let zCursor = mark.appendChild(new DeafLine) as DeafLine;
    zCursor.setAttribute('class', 'cursor')
    zCursor.lineTo(0, 0);
    this.renderSVG(zCursor);


    let yCursor = mark.appendChild(new DeafLine) as DeafLine;
  ...
  private renderMask(elRoot: SVGElement) {
    let defs = document.createElementNS(NS_SVG, 'defs');
    elRoot.appendChild(defs);
    let mask = document.createElementNS(NS_SVG, 'clipPath');
    let id = Math.random().toFixed(2);
    mask.setAttributeNS(null, 'id', id);
    defs.appendChild(mask);
    let maskrect = document.createElementNS(NS_SVG, 'rect');
  ...
  private renderLabel(elRoot: SVGElement) {
    const LINEHEIGHT = 3;


    let elLabel = document.createElementNS(NS_SVG, 'g');
    elRoot.appendChild(elLabel);
    elLabel.setAttribute('id', 'label');


    let elLableBack = document.createElementNS(NS_SVG, 'rect');
    ...
    let elLableTitleBack = <SVGRectElement> elLableBack.cloneNode() ;
  ...
  renderTab(elRoot: SVGElement) {
    let elText = document.createElementNS(NS_SVG, 'text');
    let elSVG = <SVGTextPathElement> (document.createElementNS(NS_SVG, 'textPath'));
    ...
    let elDot = new DeafElement('point');
    elDot.data = "";
    ...
    let offset: number = 1
      ...
      , tgt : SVGPathElement
      , lasttarget: SVGPathElement;

    let watchList = this.watch;

    let elLb = this.renderLabel(elRoot);

    let yCursor = <SVGPathElement> this.cover.childNodes[0].childNodes[1].childNodes[2].childNodes[0];
    yCursor.setAttributeNS(null, 'class', 'cursor');

    let xCursor = <SVGPathElement> yCursor.cloneNode();
    yCursor.parentNode.appendChild(xCursor);

    let zCursor = <SVGPathElement> this.monkey.childNodes[2].childNodes[0];
    ...
    let targetIt = (evt: MouseEvent) => {
      ...
      elLb.adjust(x);
      tgt = document.getElementById(keyClose) as any || tgt;

      if ((<SVGElement>tgt).nodeName === 'path') {
        ...
        if (tgt !== lasttarget) {
          lasttarget = tgt as SVGPathElement;
  ...
  animatedSort3D(dir = true, cb?: Function) : PromiseLike<SVGChart> {
    // this.monkey.style.display = dir ? 'block': 'none';
    this.is3DActived = dir;
    return new Promise((resolve, reject) => {
      let _cb = function (evt: TransitionEvent) {
        this.removeEventListener('transitionend', _cb);
        if (cb) {cb.call(this, evt);}
      }
      ...
      for (let i = this.pages.length - 1, p: SVGSVGElement; p = this.pages[i]; i--) {
        rotate(p, i);
      }
      rotate(this.cover, 0);
      ...
      let marker = <SVGPathElement>this.mark.childNodes[0].childNodes[0];
  ...
  monkeyBusiness(pos?: number) {
    let x = Number((<SVGTextElement>this.elSVGDot.childNodes[0]).getAttribute('x'))
      , y = Number((<SVGTextElement>this.elSVGDot.childNodes[0]).getAttribute('y'))
      , rx = this.rscaleX(x);
    if (rx < 0) return 'translateZ(0px)';
    pos = (pos || x) - this.sizeLabel

    let elMonkey = <SVGPathElement>this.monkey.childNodes[1].childNodes[0];
      
    let points = ['M ', this.sizeLabel, this.height - this.sizeLabel]
    .concat(this.pages.map((svg, i)=> {
      
      // let pt = new DeafElement('point');
      let svgPath = svg.childNodes[0].childNodes[0] as SVGPathElement
        , key = svgPath.id
        , datapoin = this.watch.members[key].data[rx];

      let svgPoint = <SVGCircleElement>elMonkey.parentNode.childNodes[i + 1];
      let x = (this.Zdistance * i + this.sizeLabel).toFixed()
        , y = this.scaleY(datapoin).toFixed();
  ...
  sort(){
    let activeSvg = this.elSVGDot.parentNode as SVGSVGElement;
    this.animatedSort3D()
    .then(() => {
